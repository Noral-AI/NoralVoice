...
async def authenticate_mcp_request() -> UserModel:
    """Resolve the authenticated NoralVoice user for an MCP tool invocation.
    ...
    and user identifiers. `_OrgRoutingExporter` reads `noralvoice.org_id`
    ...
    if span.is_recording():
        ...
        # Intentionally NOT `noralvoice.org_id` — that attribute triggers the
