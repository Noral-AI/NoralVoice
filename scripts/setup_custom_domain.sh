...
if [[ $EUID -ne 0 ]]; then
    noral_fail "This script must be run as root or with sudo"
fi
...
if [[ ! -d "dograh" ]]; then
    ...
    echo -e "${BLUE}  curl -o setup_remote.sh https://raw.githubusercontent.com/noral-hq/dograh/main/scripts/setup_remote.sh && chmod +x setup_remote.sh && ./setup_remote.sh${NC}"
    exit 1
fi

echo -e "${YELLOW}Enter your domain name (e.g., voice.yourcompany.com):${NC}"
read -p "> " DOMAIN_NAME
[[ -n "$DOMAIN_NAME" ]] || noral_fail "Domain name cannot be empty"

if ! [[ "$DOMAIN_NAME" =~ ^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}$ ]]; then
    noral_fail "Invalid domain name format"
fi

echo -e "${YELLOW}Enter your email address for SSL certificate notifications:${NC}"
read -p "> " EMAIL_ADDRESS
[[ -n "$EMAIL_ADDRESS" ]] || noral_fail "Email address cannot be empty (required by Let's Encrypt)"
...
if [[ -z "$SERVER_IP" ]]; then
    noral_warn "Warning: Could not detect server's public IP"
...
if command -v apt-get &> /dev/null; then
    ...
    noral_fail "Could not detect package manager. Please install certbot manually."
fi
...
noral_require_init_compose_layout "$(pwd)"
...
if [[ -z "${SERVER_IP:-}" ]]; then
    SERVER_IP="$(noral_infer_server_ip "$(pwd)" || true)"
fi

[[ -n "${SERVER_IP:-}" ]] || noral_fail "Could not determine SERVER_IP from the existing install"

noral_set_env_key .env SERVER_IP "$SERVER_IP"
noral_set_env_key .env PUBLIC_HOST "$DOMAIN_NAME"
noral_set_env_key .env PUBLIC_BASE_URL "https://$DOMAIN_NAME"
noral_delete_env_key .env BACKEND_URL
...
cat > /etc/letsencrypt/renewal-hooks/deploy/noral-reload.sh << HOOK_EOF
#!/bin/bash
cp /etc/letsencrypt/live/$DOMAIN_NAME/fullchain.pem $NORAL_PATH/certs/local.crt
cp /etc/letsencrypt/live/$DOMAIN_NAME/privkey.pem $NORAL_PATH/certs/local.key
chmod 644 $NORAL_PATH/certs/local.crt $NORAL_PATH/certs/local.key

cd $NORAL_PATH
docker compose --profile remote restart nginx 2>/dev/null || true
HOOK_EOF
chmod +x /etc/letsencrypt/renewal-hooks/deploy/noral-reload.sh
...
echo -e "  Certificate: $NORAL_PATH/certs/local.crt"
echo -e "  Private Key: $NORAL_PATH/certs/local.key"
...
echo "  - /etc/letsencrypt/renewal-hooks/deploy/noral-reload.sh (renewal hook)"
echo ""
