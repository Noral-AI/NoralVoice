...
[[ -f docker-compose.yaml ]] || noral_fail "docker-compose.yaml not found in $(pwd)"
[[ -f .env ]] || noral_fail ".env not found in $(pwd)"
...
if [[ -f docker-compose.override.yaml ]]; then
    ...
    echo ""
    echo -e "${YELLOW}See https://docs.noral.ai/deployment/update#updating-a-source-build${NC}"
    exit 1
fi
...
noral_load_env_file .env

[[ -n "${TURN_SECRET:-}" ]] || noral_fail "TURN_SECRET not found in .env"
...
if [[ -z "$TARGET_VERSION" ]]; then
    noral_info "Fetching latest release tag from GitHub..."
    LATEST_TAG=$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null \
        | grep -E '"tag_name":' | head -1 \
        | sed -E 's/.*"tag_name":[[:space:]]*"([^"]+)".*/\1/' || true)

    if [[ -z "$LATEST_TAG" ]]; then
        noral_warn "Could not auto-discover latest tag - defaulting to 'main'."
        LATEST_TAG="main"
    fi
    ...
    if [[ -t 0 ]]; then
        ...
        echo -e "${YELLOW}full git tag (noral-v1.28.0), or 'main' for bleeding edge.${NC}"
...
if [[ "$TARGET_VERSION" == "latest" ]]; then
    TARGET_VERSION=$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null \
        | grep -E '"tag_name":' | head -1 \
        | sed -E 's/.*"tag_name":[[:space:]]*"([^"]+)".*/\1/' || true)
    [[ -n "$TARGET_VERSION" ]] || noral_fail "could not resolve 'latest' to a release tag"
fi
...
case "$TARGET_VERSION" in
    ...
    noral-*)
        ;;
    v*)
        TRY_TAGS+=("noral-$TARGET_VERSION")
        ;;
    *)
        TRY_TAGS+=("noral-v$TARGET_VERSION" "v$TARGET_VERSION" "noral-$TARGET_VERSION")
        ;;
esac

noral_info "Validating target version: $TARGET_VERSION..."
RESOLVED_TAG=""
...
[[ -n "$RESOLVED_TAG" ]] || noral_fail "could not find a git tag matching '$TARGET_VERSION'"

if [[ "$RESOLVED_TAG" != "$TARGET_VERSION" ]]; then
    noral_success "✓ Resolved '$TARGET_VERSION' to git tag '$RESOLVED_TAG'"
fi
...
case "$TARGET_VERSION" in
    noral-v*) IMAGE_TAG="${TARGET_VERSION#noral-v}" ;;
...
if [[ -n "$IMAGE_TAG" ]]; then
    if curl -fsI "https://hub.docker.com/v2/repositories/noralai/noral-api/tags/$IMAGE_TAG/" >/dev/null 2>&1; then
        noral_success "✓ Image tag :$IMAGE_TAG found on Docker Hub"
    else
        noral_warn "Warning: image tag :$IMAGE_TAG not found on Docker Hub - leaving images at :latest"
        IMAGE_TAG=""
    fi
fi
...
echo -e "  Server IP:        ${BLUE}$(noral_infer_server_ip "$(pwd)" || echo "unknown")${NC}"
...
if [[ -t 0 && "${NORAL_UPDATE_YES:-}" != "1" ]]; then
...
noral_download_remote_support_bundle "$(pwd)" "$TARGET_VERSION"
rm -f nginx.conf turnserver.conf

if [[ -n "$IMAGE_TAG" ]]; then
    sed -i.tmp -E "s#(noral-(api|ui)):latest#\1:$IMAGE_TAG#g" docker-compose.yaml
    rm -f docker-compose.yaml.tmp
    noral_success "✓ docker-compose.yaml updated; images pinned to :$IMAGE_TAG"
else
    noral_success "✓ docker-compose.yaml updated (image tags left at :latest)"
fi

echo -e "${BLUE}[3/3] Synchronizing environment and validating init-based remote config...${NC}"
noral_set_env_key .env FASTAPI_WORKERS "$FASTAPI_WORKERS"
noral_prepare_remote_install "$(pwd)"
docker compose config -q
noral_success "✓ Remote init configuration validated"
...
echo ""
echo -e "  ${BLUE}for f in docker-compose.yaml nginx.conf turnserver.conf .env remote_up.sh scripts/run_init.sh scripts/lib/setup_common.sh deploy/templates/nginx.remote.conf.template deploy/templates/turnserver.remote.conf.template; do${NC}"
