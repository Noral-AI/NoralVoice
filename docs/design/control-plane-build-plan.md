...
The 14 existing Synthflow subaccounts (Ethos Residential Services, Affordable Solar, MET Marketing, I-DIEM, Allure, Aspire Medspa, Academy Prep, Energy Harbor, The Trench Academy, John A Williams, Southern States Roofing, Hometown Roofing, TBRS, Florida Made Tiny Homes) are the **client list**, and map one-to-one to `organizations` rows regardless of how many ElevenLabs workspaces sit behind them. Seven have Twilio active.

⚠️ **Corrected 2026-08-08 — the subaccounts are empty** ([capability spike §3.4](./control-plane-phase-0.6-capability-spike.md)). All 91 agents and all 14 phone numbers live in the **parent** workspace; not one agent sits in a client subaccount, and the MCP credential cannot even read into them. So the subaccount list is a good starting point for *who the clients are* and tells us nothing about *where their agents live*. Client attribution actually runs through **agent naming convention**, and the convention is inconsistent (`THA - Danielle`, `Ethos Residential - …`, bare `Aspire Medspa`, unprefixed `Leverage`). **Mapping agents to organizations is therefore a manual, human-reviewed exercise, not a field copy** — it belongs in Phase 4 preparation, built and checked once, rather than being discovered mid-cutover. Several live agents (Heal Pros, Ice Machine, Cedric's Personal Assistant) map to no subaccount at all and need someone to say whether they are clients or internal.
...
5. ~~**Per-agent complexity profile**~~ ✅ **done 2026-08-08** — [capability spike §3](./control-plane-phase-0.6-capability-spike.md). **Only 13 of 91 agents have a phone number** — see the Phase 4 note below. Max extractors on any one agent is **17, under the 25 cap**, so nothing needs splitting. Extractors are a **shared library** reused across agents, so the §8 contract is per-slug, not per-agent. Ranked order in spike §3.7; the gate client is **contested** — see §12.11.
