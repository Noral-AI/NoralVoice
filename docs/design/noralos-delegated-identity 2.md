...
### PR 1 — NoralOS: actor identity + delegation flag

**Repo:** `NoralOS-canonical`

Changes:
- `ToolRunContext` type: add `triggeredByUserId: string`, `triggeredByUserEmail?: string`.
- The orchestration layer that builds `ToolRunContext` for plugin workers: populate these from the run record (must already track triggering user; verify during implementation).
- `packages/plugins/noralai-noralvoice/src/noralvoice-client.ts`: include `X-Noralos-Actor-User-Id` and `X-Noralos-Actor-User-Email` in `buildHeaders()`.
- `packages/db` schema: add `delegationCapable: boolean` to API key table (default false). Migration.
- Admin path to flip the flag (UI toggle or CLI) — minimum: a `pnpm script` that takes a key ID and sets the flag.
- Mark the production voice service key as `delegation_capable=true` after deploy.

Tests:
- `ToolRunContext` populated correctly.
- Headers sent on outgoing requests.
...
Changes:
- Migration: add `delegation_capable boolean NOT NULL DEFAULT false` to `api_keys` table.
- [`api/db/models.py`](../../api/db/models.py): `ApiKey.delegation_capable` column.
- [`api/services/auth/depends.py:193`](../../api/services/auth/depends.py) `_handle_api_key_auth`: implement the decision tree above.
- New helper `_resolve_delegated_actor(api_key, request)` returns the JIT-provisioned user or None.
- Reuse [`db_client.get_or_create_user_by_provider_id`](../../api/db/client.py) — same call browser SSO uses.

Tests (`api/tests/auth/test_delegated_identity.py`):
- Delegation-capable key + valid actor headers → returns JIT-provisioned user (new and existing).
- Delegation-capable key + no actor headers → returns api_key.created_by (fallback).
- Non-delegation key + actor headers → ignores headers, returns api_key.created_by (no spoofing).
- Email sync only happens on JIT creation; existing users' emails are not overwritten.
- Same NoralOS user ID via browser SSO and via delegated assertion → same NoralVoice user row.
