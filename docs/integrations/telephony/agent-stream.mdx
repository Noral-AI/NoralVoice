...
- A Dograh API key for the same organization that owns the agent — generated under **Settings → API Keys** in the dashboard.

## Authentication

The WebSocket upgrade requires `?api_key=<value>` on the URL. The key must belong to the same organization as the agent; otherwise the server closes the socket with close code **4401**. The agent UUID alone is **not** sufficient for authentication — UUIDs can leak through exported workflow JSON and are not secrets.
...
| --- | --- | --- |
| `api_key` | Yes | Dograh API key for the agent's organization. See [Authentication](#authentication). |
...
<Response>
  <Connect>
    <Stream url="wss://app.dograh.com/api/v1/agent-stream/{agent_uuid}?api_key=...&provider=cloudonix&Domain=...&callId=...&from=...&to=..."/>
  </Connect>
  <Pause length="40"/>
</Response>
```
...
| `4401` | Missing or invalid `api_key`, or the key's organization does not match the agent's |
...
- Treat the URL as a secret — it embeds your `api_key`. Store and transmit it only over TLS, and avoid logging the raw URL in places where access is broader than your operations team.
- Rotate the `api_key` if a URL is exposed; revoking the key invalidates every stream that uses it.
