                ...
                const response = await createOrUpdateEmbedTokenApiV1WorkflowWorkflowIdEmbedTokenPost({
                    ...
                    body: {
                        ...
                        settings: {
                            ...
                            size: "medium",
                            autoStart: false,
                            containerId: embedMode === "inline" ? "noralvoice-inline-container" : undefined,
                        },
                        usage_limit: null,
                                        ...
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                ...
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    ...
                                                    <li>• Call <code className="text-xs">window.NoralVoiceWidget.start()</code> to begin a call.</li>
                                                    <li>• Call <code className="text-xs">window.NoralVoiceWidget.end()</code> to end it.</li>
...
window.NoralVoiceWidget?.onStatusChange((status) => {
  callStatus = status;
...
document.getElementById('talk-btn').addEventListener('click', () => {
  if (callStatus === 'connected' || callStatus === 'connecting') {
    window.NoralVoiceWidget.end();
  } else {
    window.NoralVoiceWidget.start();
  }
});`}</code>
                                                </pre>
  ...
  useEffect(() => {
    window.NoralVoiceWidget?.onStatusChange(setStatus);
  }, []);

  const isLive = status === 'connected' || status === 'connecting';
  return (
    <button onClick={() => isLive ? window.NoralVoiceWidget.end() : window.NoralVoiceWidget.start()}>
                                    ...
                                    {embedMode === "inline" && (
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                <h4 className="font-medium mb-2">Integration Instructions</h4>
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    <li>• Add a div with id=&quot;noralvoice-inline-container&quot; where you want the widget</li>
                                                    <li>• The widget will render inside this container</li>
                                                    <li>• You have full control over the container&apos;s styling</li>
                                                    <li>• Call window.NoralVoiceWidget.start() to begin the call</li>
                                                    <li>• Call window.NoralVoiceWidget.end() to end the call</li>
                                                </ul>
                                            </div>
                                            ...
                                            <div className="rounded-lg bg-blue-50 dark:bg-blue-950/20 p-4 border border-blue-200 dark:border-blue-800">
                                                ...
                                                <pre className="text-xs overflow-x-auto">
                                                    <code className="text-blue-800 dark:text-blue-200">{`export function NoralVoiceAgent() {
  const [isCallActive, setIsCallActive] = useState(false);

  useEffect(() => {
    // Widget will auto-initialize when script loads
    window.NoralVoiceWidget?.onCallStart(() => {
      setIsCallActive(true);
    });
    window.NoralVoiceWidget?.onCallEnd(() => {
  ...
  return (
    <div className="my-8">
      ...
      <div id="noralvoice-inline-container" className="min-h-[400px]">
        {/* Widget renders here */}
      </div>
      <button
        onClick={() => window.NoralVoiceWidget?.start()}
