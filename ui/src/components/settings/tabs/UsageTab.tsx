...
const USAGE_TAB_ID = "usage-billing";
...
export default function UsageTab() {
  ...
  const [activeFilters, setActiveFilters] = useState<ActiveFilter[]>(() =>
    decodeFiltersFromURL(searchParams, usageFilterAttributes),
  );
  ...
  const fetchMpsCredits = useCallback(async () => {
    ...
    try {
      ...
      if (response.data) setMpsCredits(response.data);
    } catch (error) {
  ...
  const fetchUsageHistory = useCallback(
    async (page: number, filters?: ActiveFilter[]) => {
      if (!auth.isAuthenticated) return;
      setIsLoadingHistory(true);
      try {
        const response = await getUsageHistoryApiV1OrganizationsUsageRunsGet({
          query: { page, limit: 50, ...buildUsageQueryParams(filters) },
        });
        if (response.data) setUsageHistory(response.data);
      } catch (error) {
  ...
  const fetchDailyUsage = useCallback(async () => {
    ...
    try {
      ...
      if (response.data) setDailyUsage(response.data);
    } catch (error) {
  ...
  useEffect(() => {
    if (!userConfigLoading) {
      if (userConfig?.timezone) setSelectedTimezone(userConfig.timezone);
      else setSelectedTimezone(localTimezone);
    }
  ...
  // Push URL state for page + filters while remaining on /settings?tab=usage-billing.
  // Old /usage?... URLs are handled by the 301 in next.config.ts.
  const updateUrlParams = useCallback(
    (params: { page?: number; filters?: ActiveFilter[] }) => {
      const newParams = new URLSearchParams();
      newParams.set(SETTINGS_TAB_PARAM, USAGE_TAB_ID);
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            See all your Agent Runs across all Voice Agents. You can use filters to
            narrow down to specific runs.
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>These track usage of the model proxy service.</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoadingCredits ? (
            ...
            <div className="space-y-4">
              ...
              {mpsCredits.total_quota > 0 && (
                <Progress
                  value={(mpsCredits.total_credits_used / mpsCredits.total_quota) * 100}
                  className="h-3"
                />
              )}
            </div>
      ...
      {organizationPricing?.price_per_second_usd && (
        <DailyUsageTable data={dailyUsage} isLoading={isLoadingDaily} />
      )}
            ...
            <>
                    ...
                    {usageHistory.runs.map((run) => (
                      <TableRow key={run.id}>
                        ...
                        <TableCell className="text-sm">
                          {(run.call_type === "inbound" ? run.caller_number : run.called_number) || "-"}
                        </TableCell>
                        <TableCell>
                          {run.disposition ? (
              ...
              {usageHistory.total_pages > 1 && (
                <div className="mt-6 flex items-center justify-between">
                  <p className="text-sm text-muted-foreground">
                    Page {usageHistory.page} of {usageHistory.total_pages} (
                    {usageHistory.total_count} total runs)
                  </p>
